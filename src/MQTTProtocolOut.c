...
#endif
{
	...
	if (aClient->net.http_proxy) {
		...
		if (timeout < 0)
			...
			rc = Socket_new(address, addr_len, port, &(aClient->net.socket), timeout, aClient->tcpUserTimeoutMs);
#else
		rc = Socket_new(address, addr_len, port, &(aClient->net.socket), aClient->tcpUserTimeoutMs);
#endif
	}
